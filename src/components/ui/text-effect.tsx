...
const presetVariants: Record<PresetType, {
  container: Variants;
  item: Variants;
}> = {
  blur: {
...
const AnimationComponent: React.FC<{
  ...
  const content = per === 'line' ? <motion.span variants={variants} className='block'>
        {segment}
      </motion.span> : per === 'word' ? <motion.span aria-hidden='true' variants={variants} className="inline-block whitespace-pre px-0 py-[20px] text-center text-5xl font-light">
        {segment}
  ...
  return <span className={cn(defaultWrapperClassName, segmentWrapperClassName)}>
      {content}
    </span>;
});
...
export function TextEffect({
  ...
  return <AnimatePresence mode='popLayout'>
      {trigger && <MotionTag initial='hidden' animate='visible' exit='exit' aria-label={ariaLabel} variants={delayedContainerVariants} className={cn('whitespace-pre-wrap', className)} onAnimationComplete={onAnimationComplete}>
          {segments.map((segment, index) => <AnimationComponent key={`${per}-${index}-${segment}`} segment={segment} variants={itemVariants} per={per} segmentWrapperClassName={segmentWrapperClassName} />)}
        </MotionTag>}
    </AnimatePresence>;
}